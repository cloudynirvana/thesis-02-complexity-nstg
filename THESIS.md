...
**Thesis #2 -- computational research thesis**  
**Author:** Kelechi Emeka Ogbonna  
**Date:** September 2026  
**Format:** B.Sc. project chapter structure (Nile University style) for journal / thesis handoff  
**Kind:** Computational research thesis manuscript. **Not** a clinical study. **Not** a wet-lab study.  
...
I, Kelechi Emeka Ogbonna, hereby declare that this computational research thesis titled "Complexity Science and NSTG-Guided In-Silico Pathology Dynamics for Biologics Pathway Exploration" was carried out by me. The findings reported here are in-silico and architectural. Nigeria Standard Treatment Guidelines (NSTG 2022) appear only as a qualitative knowledge constraint. They are not ODE coefficients, doses, or a care protocol. No patient data were used. No DOI has been invented.
...
The work reported here is an architectural computational thesis, not a clinical study. It specifies, implements, and tests a data-only in-silico pipeline. A contributor authors a YAML `CaseCard` -- disease framing, systemic axes, research-only observables, candidate mechanisms, falsifiers, qualitative NSTG touchpoints, Vancouver citations, and a required honesty disclaimer. The card is refused if it carries doses, rate constants, ODE knobs, PK/PD tokens, numeric leaves, or NSTG-derived numeric scales. A stub `PathwaySketch` explorer then ranks bibliographic, falsifiable hypotheses, lists evidence still required, and emits an explicit `refused_non_parameters` record. The evidence gate never opens. NSTG 2022 is used only as a structured clinical-knowledge constraint layer [49,50]. It is never executed as a care protocol and never auto-translated into model parameters.

Four seed CaseCards were constructed from published knowledge and evidence pointers, not from patient records: triple-negative breast cancer (TNBC) framed as metabolic-immune exclusion [23,28-30]; glioblastoma (GBM) framed as an invasive hypoxic niche [31-34]; pancreatic ductal adenocarcinoma (PDAC) framed as a stromal barrier [37-41]; and dormant / occult residual disease framed explicitly as a research hypothesis space [45-48]. Each card validates against schema version 1.0.0. Across the four cards the explorer emits thirteen ranked hypotheses. Every hypothesis has `parameter_status = refused` and `prediction_status = not_emitted`.

The key objectives of this research study are to:

i. Enforce epistemic separation so that a pathway hypothesis cannot be written as a coefficient by accident  
ii. Define a CaseCard as a reusable research object with falsifiers and qualitative NSTG constraints  
iii. Use NSTG 2022 only as a knowledge constraint, never as ODE coefficients  
iv. Rank bibliographic hypotheses while refusing parameterisation and prediction  
v. Instantiate four seed cases (TNBC, GBM, PDAC, dormancy) without editing a solver

Findings from this study are architectural. No ODE was integrated in this pipeline. No public dataset was analysed. No effect size, survival, response, or care pathway is claimed. Named public resources are listed only as future binds [54-63]. The contribution is a reusable research object and a closed gate.

**This thesis is research only. It is not a medical device, not CDS, not dosing, and not a wet-lab antidiabetic study.**

---

## Keywords

complexity science; organised complexity; in-silico pathology; systemic control; Nigeria Standard Treatment Guidelines (NSTG); knowledge constraint; CaseCard; PathwaySketch; biologics pathway exploration; parameter smuggling; evidence gate; computational research thesis
...
CHAPTER TWO -- LITERATURE REVIEW  
    2.1 Complexity science and organised complexity  
    2.2 Complexity in health systems  
    2.3 Why model -- and when not to  
    2.4 Complex pathology as coupled niches  
    2.5 NSTG 2022 as knowledge constraint  
    2.6 Complementary objects in the source repository  

CHAPTER THREE -- MATERIALS AND METHODS  
    3.1 Study design  
    3.2 CaseCard schema  
    3.3 Parameter-smuggling refusal  
    3.4 NSTG constraint layer  
    3.5 Evidence gate  
    3.6 PathwaySketch explorer  
    3.7 Seed-case construction  
    3.8 Honesty controls  
    3.9 What was not done  

CHAPTER FOUR -- RESULTS  
    4.1 Four CaseCards validate as research objects  
    4.2 PathwaySketch ranking is bibliographic, not predictive  
    4.3 Parameter-smuggling refusal  
    4.4 The evidence gate stays closed on clean cards  
    4.5 Packages do not import an ODE stack  
    4.6 What the scores are not  
...
**Table 3-1.** CaseCard required fields and their epistemic layer.  
**Table 4-1.** Seed CaseCard field counts (schema 1.0.0).  
**Table 4-2.** Parameter-smuggling mutations refused by automated tests.  
**Table 4-3.** Future public-dataset binds (not analysed in this thesis).  
...
Relation to a companion modelling experiment

Pull request #1 in the source repository explores a compact multi-scale host-burden ODE with numeric constraint *hints*. This manuscript describes the complementary **data-first** layer: YAML CaseCards, qualitative NSTG constraints, and an evidence gate that **never** auto-translates NSTG into ODE coefficients. Numeric scales such as `x_cap_scale` and `infection_risk_weight` are treated here as smuggled non-parameters.
...
The aim of this research is to specify, implement, and test a data-first in-silico architecture in which complex pathological cases can be authored as YAML research objects under NSTG as a qualitative knowledge constraint, without auto-translating guidelines into ODE coefficients, doses, or predictions.

The objectives of this research study are to:

i. State and enforce a layered boundary -- knowledge, evidence, mechanism, parameter, prediction -- so that a biologics pathway hypothesis cannot be written as a coefficient by accident  
ii. Define a strict CaseCard schema in which a complex pathological case is authored as YAML, with falsifiers, qualitative NSTG touchpoints, and Vancouver citations that carry real DOIs for primary and review items  
iii. Use NSTG 2022 and adjacent Nigerian policy documents only as a structured clinical-knowledge constraint layer [49-53]; cite them; do not copy them; do not map them to `x_cap`, infection-risk weights, doses, or rate constants  
iv. Implement a PathwaySketch explorer that ranks hypotheses from citations, falsifiers, and qualitative NSTG flags, lists evidence still required, and always refuses parameterisation and prediction  
v. Instantiate four complex cases -- TNBC metabolic-immune exclusion, GBM invasive hypoxic niche, PDAC stromal barrier, dormant/occult residual disease -- as starting research objects with falsifiers  
vi. Show that adding a case is a data-only act (typical local cost: under fifteen minutes plus validation) and that the `pathology_cases` and `pipeline` packages do not import `numpy`, `scipy`, or any ODE module
...
**What this significance is not.** This thesis does not diagnose, treat, prevent, or cure anyone. It is not a medical device and not CDS. It does not recommend a biologic product, dose, schedule, or combination. It does not claim that a pathway has been found. Ranked hypotheses are bibliographic starting points awaiting independent falsification. It is not a wet-lab claim about Carica papaya or silver nanoparticles.
...
The manuscript reports what the software does on those objects. It does not analyse patient data, does not fit parameters, and does not bind public datasets.
...
This is a computational methods and architecture study. There is no human-subjects protocol, no animal protocol, no chart review, and no wet-lab bench work. Inputs are YAML research cards and published bibliographic identifiers. Outputs are validation results, PathwaySketch JSON objects, and this manuscript. The analysis unit is the **card**, not the patient.
...
**Table 3-1. CaseCard required fields**
...
- phytochemical analysis, silver-nanoparticle synthesis, or alpha-amylase inhibition (those belong to Thesis 0, a separate wet-lab B.Sc. project).
...
**Table 4-1. Seed CaseCard field counts**
...
**Table 4-2. Parameter-smuggling mutations refused**
...
Hallmarks [18,19], accessory cells [20], microenvironmental regulation [21,22], exclusion [23], the immunity cycle and set point [24,25], and immunotherapy's practical challenges [26] are the knowledge background of the seed cards. The cards do not re-derive those reviews. They point at them and then ask for a falsifier. Schmid et al. [30], Brat et al. [32], Olive et al. [37], and Provenzano et al. [38] are the primary-evidence pointers. They remain pointed-at. They are not re-analysed.

It is unfashionable to treat a disclaimer as a result. In this domain the disclaimer is doing scientific work. Ioannidis and Begley-Ellis described environments in which a figure is treated as a fact [13,14]. A PathwaySketch that omitted `refused_non_parameters` would invite that treatment. The standing refusals on clean cards exist so that a reader of the JSON cannot say "the gate was silent, therefore I may fit." Silence is not consent.
...
- **Stub ranking.** `research_score` is a small integer heuristic. It is sensitive to how many citations an author attached, not to citation quality beyond "has a real DOI."  
- **NSTG is not ingested.** The constraint layer is a short original catalog plus author-written touchpoint sentences. Completeness belongs to FMoH, not to this repository [49,50].  
- **Theme matching is lexical.** Infection-ish detection is substring matching on touchpoint `theme` and `id`.  
- **No identifiability path.** Even if the gate were opened -- it is not -- the cards do not contain the measurements Aldridge et al. required for a physicochemical model [12].  
- **English-language, oncology-weighted seeds.** The four cases are not a map of Nigerian cancer incidence.  
- **Companion ODE is out of scope.** Results here cannot be read as a validation or a refutation of the MHBD-4 model in pull request #1.  
- **Single author, computational setting.** This is a sole-author computational manuscript. It has not undergone journal peer review at the time of deposit.
...
This thesis contributes a data-first in-silico architecture, implemented and tested in the source repository, in which:

i. a CaseCard holds knowledge, evidence pointers, mechanisms, falsifiers, and qualitative NSTG constraints;  
ii. NSTG 2022 is cited as a knowledge constraint and is never auto-translated into ODE coefficients [49,50];  
iii. four seed cases (TNBC, GBM, PDAC, dormancy) stand as research objects with real, Crossref-checked citations;  
iv. a PathwaySketch explorer ranks bibliographic hypotheses and always refuses parameters and predictions;  
v. parameter smuggling, including PR #1 numeric scales, is a failing test rather than a hidden feature.

The work is computational research. It is not a medical device, not CDS, not dosing, and not a cure. Named public datasets are future binds, not silent analyses. Official guideline text remains with its publisher. This computational thesis is not a wet-lab diabetes claim.
...
i. If a later study opens an evidence record -- a new object, not a CaseCard field -- the following named public resources are the intended first binds. They are listed so that a future author cannot invent a private cohort in the gap. They are not used in Results.

**Table 4-3. Future public-dataset binds (not analysed)**

| Intended bind | Why it matches a seed motif | Citation |
| --- | --- | --- |
| TCGA breast cohort (TCGA-BRCA) molecular portraits | TNBC heterogeneity as published knowledge | [54] |
...
| NCBI Gene Expression Omnibus | Functional-genomics archive | [60,61] |
| Ivy Glioblastoma Atlas Project | Anatomic transcriptional atlas of human GBM niches | [62] |
| Human Tumor Atlas Network | Spatial and temporal tumour transitions | [63] |
| ICGC/TCGA Pan-Cancer Analysis of Whole Genomes | Whole-genome context if a future evidence record needs it | [64] |

ii. A CaseCard may *point* at a dataset accession. It may not contain a fitted number from that dataset. NSTG still does not become a coefficient. Negative assays remain detection limits, not proof of clearance (dormancy card). Animal delivery studies remain animal evidence (PDAC card).  
iii. A future schema may introduce an `EvidenceRecord` with measured observables and an explicit `gate: still_closed` default. Until that object exists, the stub is the specification.  
iv. The catalog can grow as original theme lists -- malaria, sickle cell, tuberculosis, febrile neutropenia -- without becoming a shadow NSTG. Any growth that copies official tables is out of scope and out of licence.  
v. Ranking can be replaced by a documented multi-criteria rule without introducing units of mg or h−1. If an ODE is ever coupled, it should consume an EvidenceRecord, not a CaseCard.  
vi. Additional organised-complexity motifs should be new YAML files. They should not be new Python packages.

---

## REFERENCES

Vancouver / NLM. Journal items 1-64 were completed from PubMed MEDLINE on 20 September 2026; items 65-69 were completed the same way on 21 September 2026. Printed DOIs were resolved on the Crossref Works API on those dates; unverified DOIs are not printed. Guideline and policy items are Internet citations with a cited date. Books have no DOI. No DOI was invented.
...
This work does not diagnose, treat, prevent, cure, or manage any person. It does not recommend a biologic product, dose, schedule, or combination. Ranked items are computational hypotheses awaiting independent falsification.

No patient data were used. Public datasets named in Recommendations were not analysed. The author does not speak for the Federal Ministry of Health, Nigeria, or for any manufacturer or regulator.
